...
Whereas the Kalman Filter represents the belief as a paramaterised gaussian distribution, the Particle Filter is non-parameterised, representing the belief as a finite set $\chi$ of $M$ states drawn from the state space according to the distribution of the posterior. By increasing the size of the particle filter, this can be made to approximate any distribution without any regards to constraints such as unimodality.

At the start of the algorithm, the set of particles $\chi_0$ are drawn according to the distribution of the start state $p(x_0)$. 

At time $t$, for each $i \in [1,M]$ a particle $\bar{x}^{[i]}_t$ is drawn from the distribution of the state transition probability $p(x_t | u_t,x^{[i]}_{t-1})$ into the set $\bar{\chi}_t$. Next, draw $M$ particles from $\bar{\chi}_t$ into $\chi_t$ with probability $\eta p(z_t | x^{[i]}_t)$.
